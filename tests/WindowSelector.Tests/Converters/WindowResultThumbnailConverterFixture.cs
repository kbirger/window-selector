using System;
using System.Globalization;
using System.Threading;
using System.Windows;
using System.Windows.Threading;
using NUnit.Framework;
using WindowSelector.Common.ViewModels;
using WindowSelector.Converters;
using WindowSelector.Plugins.BrowserTabs.ViewModels;
using WindowSelector.Plugins.Win32.Models;
using WindowSelector.Plugins.Win32.ViewModels;
using WindowSelector.Win32;

namespace WindowSelector.Tests.Converters
{
    [TestFixture]
    public class WindowResultThumbnailConverterFixture
    {
        private Autofac.Extras.Moq.AutoMock mock;

        [Test]
        public void Test()
        {
            Assert.Fail("You need to uncomment these tests");
        }
        //[SetUp]
        //public void Setup()
        //{
        //    mock = Autofac.Extras.Moq.AutoMock.GetLoose();
        //    mock.Provide(Dispatcher.CurrentDispatcher);
        //}

        //[TearDown]
        //public void Teardown()
        //{
        //    mock.Dispose();
        //    mock = null;
        //}


        //[Test]
        //[Apartment(ApartmentState.STA)]
        //public void Test_null_yields_null()
        //{
            
        //    var converter = new WindowResultThumbnailConverter();

        //    // Act
        //    var ret = converter.Convert(null, typeof(UIElement), null, CultureInfo.CurrentCulture);

        //    // Assert
        //    Assert.That(ret, Is.Null);
        //}

        //[Test]
        //[Apartment(ApartmentState.STA)]
        //public void Test_Convert_nativeWindowResult()
        //{
        //    mock.Provide(new NativeWindowInfo()
        //    {
        //        hWnd = new IntPtr(123)
        //    });
        //    var windowResult = mock.Create<NativeWindowResult>();
        //    var converter = new WindowResultThumbnailConverter();

        //    // Act
        //    var ret = converter.Convert(windowResult, typeof(UIElement), null, CultureInfo.CurrentCulture);

        //    // Assert
        //    Assert.That(ret, Is.InstanceOf<UIElement>());
        //}
        //[Test]
        //[Apartment(ApartmentState.STA)]
        //public void Test_Convert_tabWindowResult()
        //{
        //    mock.Provide(new TabInfo()
        //    {
        //        Thumb = "data:image/x-icon;base64,"
        //    });
        //    var windowResult = mock.Create<TabWindowResult>();
        //    var converter = new WindowResultThumbnailConverter();

        //    // Act
        //    var ret = converter.Convert(windowResult, typeof(UIElement), null, CultureInfo.CurrentCulture);

        //    // Assert
        //    Assert.That(ret, Is.InstanceOf<UIElement>());
        //}

        //[Test]
        //[Apartment(ApartmentState.STA)]
        //public void Test_Convert_noThumb()
        //{
        //    // Arrange
        //    mock.Provide(new TabInfo()
        //    {
        //        Thumb = null
        //    });
        //    var noThumb = mock.Create<TabWindowResult>();
        //    var converter = new WindowResultThumbnailConverter();

        //    // Act
        //    var ret = converter.Convert(noThumb, typeof(UIElement), null, CultureInfo.CurrentCulture);

        //    // Assert
        //    Assert.That(ret, Is.InstanceOf<UIElement>());
        //}

        //[Test]
        //public void Test_ConvertBack()
        //{
        //    // Arrange
        //    var converter = new WindowResultThumbnailConverter();
        //    // Assert
        //    Assert.That(()=> converter.ConvertBack("anything", typeof(WindowResult), null, CultureInfo.CurrentCulture), Throws.InstanceOf<NotImplementedException>());
        //}
    }
}
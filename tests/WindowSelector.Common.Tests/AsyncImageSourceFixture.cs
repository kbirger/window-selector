using NUnit.Framework;
using System;
using System.Drawing;
using System.IO;
using System.Reflection;
using System.Security.Permissions;
using System.Threading;
using System.Threading.Tasks;
using System.Windows.Threading;
using WindowSelector.Common;

namespace WindowSelector.Tests
{
    // todo: bleongs with non cross platform code
    [TestFixture]
    public class AsyncImageSourceFixture
    {
        [OneTimeSetUp]
        public void OnTestInitialize()
        {
            if (!UriParser.IsKnownScheme("pack")) new System.Windows.Application();
        }

        [Test]
        public void Test_file_image_source_works()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);
            var imagePath = Path.GetTempFileName();

            ResourceUtil.LoadTestIconToPath(imagePath);

            var handle = new AutoResetEvent(false);
            imgSource.PropertyChanged += (sender, args) =>
            {
                handle.Set();
            };
            var fileUri = new Uri(imagePath).AbsoluteUri;
            // Act

            imgSource.Source = new Uri(fileUri);
            var wasSignaled = handle.WaitOne(timeout: TimeSpan.FromSeconds(1));

            // Assert
            Assert.That(wasSignaled, Is.True);
            Assert.That(imgSource.Image.Width, Is.EqualTo(16));
            Assert.That(imgSource.Image.Height, Is.EqualTo(16));
        }
        
        [Test]
        public void Test_file_image_source_works_when_on_another_thread()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);
            var imagePath = Path.GetTempFileName();
            var handle = new AutoResetEvent(false);
            ResourceUtil.LoadTestIconToPath(imagePath);
            imgSource.PropertyChanged += (sender, args) =>
            {
                handle.Set();
            };
            var fileUri = new Uri(imagePath).AbsoluteUri;

            // Act
            Task.Run(() => imgSource.Source = new Uri(fileUri)).Wait();
            DispatcherUtil.DoEvents();
            var wasSignaled = handle.WaitOne(timeout: TimeSpan.FromSeconds(5));

            // Assert
            Assert.That(wasSignaled, Is.True);
            Assert.That(imgSource.Image.Width, Is.EqualTo(16));
            Assert.That(imgSource.Image.Height, Is.EqualTo(16));
        }

        [Test]
        public void Test_icon_image_source_works()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);
            System.Drawing.Icon icon;
            using (var imageStream = ResourceUtil.GetStream("TestIco.ico"))
            {
                icon = new Icon(imageStream);
            }
            var handle = new AutoResetEvent(false);
            imgSource.PropertyChanged += (sender, args) =>
            {
                handle.Set();
            };
            // Act

            imgSource.Source = icon;
            DispatcherUtil.DoEvents();
            var wasSignaled = handle.WaitOne(timeout: TimeSpan.FromSeconds(1));
            // Assert
            Assert.That(wasSignaled, Is.True);
            Assert.That((int)imgSource.Image.Width, Is.EqualTo(16));
            Assert.That((int)imgSource.Image.Height, Is.EqualTo(16));
        }

        [Test]
        public void Test_icon_image_source_works_on_another_thread()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);
            System.Drawing.Icon icon;
            icon = ResourceUtil.GetIcon();
            var handle = new AutoResetEvent(false);
            imgSource.PropertyChanged += (sender, args) =>
            {
                handle.Set();
            };
            // Act
            Task.Factory.StartNew(() => imgSource.Source = icon).Wait();
            DispatcherUtil.DoEvents();
            var wasSignaled = handle.WaitOne(timeout: TimeSpan.FromSeconds(1));
            // Assert
            Assert.That(wasSignaled, Is.True);
            Assert.That((int)imgSource.Image.Width, Is.EqualTo(16));
            Assert.That((int)imgSource.Image.Height, Is.EqualTo(16));
        }

        [Test]
        public void Test_set_source_twice_throws()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);
            imgSource.Source = new Uri("http://www.contoso.com/img.ico");

            // Act
            Assert.That(() => imgSource.Source = new Uri("http://www.contoso.com/img.ico"), Throws.InvalidOperationException);
        }

        [Test]
        public void Test_404_download_failed()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);
            var handle = new AutoResetEvent(false);

            imgSource.PropertyChanged += (sender, args) =>
            {
                if(imgSource.Image == null)
                    handle.Set();
            };

            // Act
            imgSource.Source = new Uri("http://www.coxxxftoso.com/img.ico");
            DispatcherUtil.DoEvents();
            var wasSignaled = handle.WaitOne(timeout: TimeSpan.FromSeconds(10));

            // Assert
            Assert.That(wasSignaled, Is.True);
            Assert.That(imgSource.Image, Is.Null);


        }

        [Test]
        public void Test_set_source_null_doesnt_trigger_work()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);
            var handle = new AutoResetEvent(false);
            imgSource.PropertyChanged += (sender, args) =>
            {
                handle.Set();
            };
            // Act
            Task.Factory.StartNew(() => imgSource.Source = null).Wait();
            DispatcherUtil.DoEvents();
            var wasSignaled = handle.WaitOne(timeout: TimeSpan.FromSeconds(1));
            // Assert
            Assert.That(wasSignaled, Is.False);
        }

        [Test]
        public void Test_uri_data_source_works()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);

            var handle = new AutoResetEvent(false);
            imgSource.PropertyChanged += (sender, args) =>
            {
                handle.Set();
            };

            // Act
            imgSource.Source = new Uri("data:image/x-icon;base64,");

            DispatcherUtil.DoEvents();
            var wasSignaled = handle.WaitOne(timeout: TimeSpan.FromSeconds(1));

            // Assert
            Assert.That(wasSignaled, Is.True);
            Assert.That(imgSource.Image.Width, Is.EqualTo(32));
        }

        [Test]
        public void Test_uri_with_invalid_scheme_throws()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);

            // Assert
            Assert.That(() => imgSource.Source = new Uri("foo://hi"), Throws.InstanceOf<NotSupportedException>());
        }

        [Test]
        public void Test_get_Source_gets_UriSource()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);
            var uri = new Uri("http://www.www.com");
            imgSource.Source = uri;

            // Act
            var actualSource = imgSource.Source;

            // Assert
            Assert.That(actualSource, Is.EqualTo(uri));
        }

        [Test]
        public void Test_get_Source_gets_IconSource()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);
            var icon = ResourceUtil.GetIcon();
            imgSource.Source = icon;

            // Act
            var actualSource = imgSource.Source;

            // Assert
            Assert.That(actualSource, Is.EqualTo(icon));
        }

        [Test]
        public void Test_source_checks_type_and_throws()
        {
            // Arrange
            var imgSource = new AsyncImageSource(Dispatcher.CurrentDispatcher);

            // Act
            Assert.That(() => imgSource.Source = 25, Throws.InstanceOf<NotSupportedException>());

        }

        private Dispatcher GetDispatcher()
        {
            var ctor = typeof(Dispatcher).GetConstructor(BindingFlags.NonPublic | BindingFlags.Instance, null, new Type[0], null);
            return (Dispatcher)ctor.Invoke(null);
        }

        private Dispatcher GetDispatcherFromOtherThread()
        {
            AutoResetEvent evt = new AutoResetEvent(false);
            Dispatcher ret = null;
            Thread thread = new Thread(() =>
            {
                bool doEvents = true;
                ret = GetDispatcher();
                ret.ShutdownStarted += (sender, args) => doEvents = false;

                evt.Set();
                while (doEvents)
                {
                    Thread.Sleep(10);
                    DispatcherUtil.DoEvents();

                }
            });
            thread.Start();
            evt.WaitOne();

            return ret;
        }
    }

    public static class DispatcherUtil
    {
        [SecurityPermission(SecurityAction.Demand, Flags = SecurityPermissionFlag.UnmanagedCode)]
        public static void DoEvents()
        {
            DispatcherFrame frame = new DispatcherFrame();
            Dispatcher.CurrentDispatcher.BeginInvoke(DispatcherPriority.Background,
                new DispatcherOperationCallback(ExitFrame), frame);
            Dispatcher.PushFrame(frame);
        }

        private static object ExitFrame(object frame)
        {
            ((DispatcherFrame)frame).Continue = false;
            return null;
        }
    }
}
